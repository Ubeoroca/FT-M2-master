...
// Tu código acá:
document.getElementById('createdBy').innerHTML += ' Ube';
...
function buildToDo(todo, index) {
  ...
  // toDoShell.className = 'TodDoShell';
...
function buildToDos(toDos) {
  // Tu código acá:
  let array = toDos.map(function(todo,index){return buildToDo(todo, index)});
  return array;
}
...
function addToDo() {
  ...
  let nuevoTodo = new ToDo(valorInput);
  toDoItems.push(nuevoTodo);
  let inputBorrado = document.getElementById('toDoInput');
  inputBorrado.value = '';
  displayToDos();
}
